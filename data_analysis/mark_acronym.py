...
def mark_english_pre(dict):
    df_old_list = df_forbidden['물리명'].astype(str).tolist()
    ...
    for word in acro_li:
        ...
        if token_word_sentence in li_ngram_token:
            ...
            if dict[ind]['char_type'] in char_morphs:
                find_location = ind_li[acro_li.index(word)]
                english_word = df_forbidden['물리명'][find_location]
                word_location = li_ngram_token.index(token_word_sentence)
                for i in range(word_location, word_location + len_word_morphs):
                    dict[i]["acronym_exist"] = 1
                    dict[i]["acronym"] = "see previous"
                dict[word_location]["acronym"] = english_word
